...
impl CacheStore {
    pub async fn get_meta(
        ...
        task::spawn_blocking(move || {
            let mut client = pool.get().map_err(|_| CacheStoreError::Disconnected)?;
            ...
            match client.hget::<_, _, (Value, Value)>(&key, (KEY_FINGERPRINT, KEY_TAGS)) {
                        ...
                        if let Value::Data(tags_bytes_data) = tags_bytes {
                            if let Ok(tags_data) = String::from_utf8(tags_bytes_data) {
                                if tags_data.is_empty() == false {
                                    ...
                                    match redis::cmd("TOUCH").arg(tags).query::<usize>(&mut *client)
                                    {
                                        Ok(bump_count) => {
                ...
                Err(_) => Err(CacheStoreError::Failed),
            }
        })
        .await
        .map_err(|_| CacheStoreError::Failed)?
    }
                    ...
                    Value::Data(body_bytes_raw) => {
                        let body_bytes_result = if APP_CONF.cache.compress_body == true {
                            ...
                            let mut decompressor =
                                BrotliDecompressor::new(&body_bytes_raw[..], 4096);
        ...
        task::spawn_blocking(move || {
            ...
            if let Ok(store_value_bytes) = store_value_bytes_result {
                ...
                for key_tag in key_tags {
                    ...
                    pipeline
                        .expire(&key_tag.0, APP_CONF.redis.max_key_expiration)
                        .ignore();
                }
    ...
    pub fn purge_tag(
        ...
        let mut client = self.pool.get().map_err(|_| CacheStoreError::Disconnected)?;
